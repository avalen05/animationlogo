document.addEventListener("DOMContentLoaded", function () {
    ...
    // Function to randomize and apply images to slots
        ...
        document.querySelectorAll('.logo-slot').forEach((element, index) => {
            setTimeout(function () {
                ...
                element.addEventListener('transitionend', function () {
                    ...
                    console.log("Applied background image to slot", index, ":", selectedLogos[index]);
                    element.classList.remove('fade-out');
                    element.classList.add('fade-in');
                }, { once: true });
            }, Math.random() * 2000); // Randomize the delay for each image
        });
    }
    ...
    // Initial call to randomize logos
    randomizeLogos();
}); // <--- This is the closing bracket for the DOMContentLoaded function
